...
from gpupath.query import _predecessors, _shortest_path, _shortest_path_lengths
...
def test_bfs_distances() -> None:
    ...
    assert _shortest_path_lengths(graph, engine, 0) == [0, 1, 1, 2, 2]
...
def test_bfs_predecessors_and_path() -> None:
    ...
    path = _shortest_path(graph, engine, 0, 4)
...
def test_unreachable_path_unweighted() -> None:
    ...
    assert _shortest_path(graph, engine, 0, 3) == []
    assert _shortest_path_lengths(graph, engine, 0) == [
        0,
        1,
...
def test_weighted_shortest_path_default_sssp() -> None:
    ...
    assert _shortest_path(graph, engine, 0, 4) == [0, 1, 2, 4]
...
def test_source_out_of_range_bfs() -> None:
    ...
    with pytest.raises(ValueError, match="source 3 out of range"):
        _shortest_path_lengths(graph, engine, 3)
...
def test_target_out_of_range() -> None:
    ...
    with pytest.raises(ValueError, match="target 5 out of range"):
        _shortest_path(graph, engine, 0, 5)
...
def test_bmssp_matches_default_sssp_on_weighted_graph() -> None:
    ...
    d_default = _shortest_path_lengths(graph, engine, 0, method="default")
    d_bmssp = _shortest_path_lengths(graph, engine, 0, method="bmssp")

    assert d_default == d_bmssp

    p_default = _shortest_path(graph, engine, 0, 5, method="default")
    p_bmssp = _shortest_path(graph, engine, 0, 5, method="bmssp")
